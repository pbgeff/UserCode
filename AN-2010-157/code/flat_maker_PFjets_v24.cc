...
int flat_maker(){
  ...
  int MC_flag = 0; 
  output_filename="CMS_PFjets_pt10_data_JMT_Jun9thReReco_v1.root";
...
/*
  ...
  int MC_flag = 0; 
  output_filename="CMS_PFjets_data_Commissioning10-SD_JetMETTau-v9.root";
*/
  ...
  for(int ia = 0; ia<nentries; ia++){ 
   ...
   //Trigger Bits
    ...
    for (unsigned int k=0; k<jets_AK5PF_pt->size(); k++) {
      if(jets_AK5PF_et->at(k)<10/*40*/) continue;
      //if(fabs(jets_AK5PF_eta->at(k))>3.0) continue;
      //write out jet data
      float et = jets_AK5PF_et->at(k);
      float phi = jets_AK5PF_phi->at(k);
      float eta = jets_AK5PF_eta->at(k);
      ...
      _jet_E[_num_jet]=  jets_AK5PF_energy->at(k);    //sqrt(_jet_et[_num_jet]*_jet_et[_num_jet]
			    //	+_jet_pz[_num_jet]*_jet_pz[_num_jet]);
      _jet_emf[_num_jet]= jets_AK5PF_chgEmE->at(k);   //jets_AK5PF_emf->at(k);
      _jet_fHPD[_num_jet]=  jets_AK5PF_neutralEmE->at(k);  //jets_AK5PF_fHPD->at(k);
      _jet_fRBX[_num_jet]=  jets_AK5PF_neutralHadE->at(k);  //jets_AK5PF_fRBX->at(k);
      _jet_n90Hits[_num_jet]=  jets_AK5PF_chg_Mult->at(k);  //jets_AK5PF_n90Hits->at(k);
      _jet_btag_jetProb[_num_jet]=  jets_AK5PF_chgHadE->at(k);  //jets_AK5PF_btag_jetProb->at(k);
